...
Before compiling on load correct module 
   module unload openmpi
   module load openmpi/1.10.2-gnu-pmi2


To compile  
   mpic++ add_num_MPI2.cpp -o add_num_MPI2

To execute:
  use run_script_mpi add_num_MPI2.sh

  where add_num_MPI2.sh is a script file that contains
     #!/bin/bash
     module unload openmpi
     module load openmpi/1.10.2-gnu-pmi2
...
void broadcast_int(int *int_num,int root, int rank, int numtasks) {
   ...
   if (rank==root) {
      for(int mpitask=0;mpitask<numtasks;mpitask++) {
         if (mpitask!=root) {
            MPI::COMM_WORLD.Send(int_num,1,MPI::INT, 
                  mpitask,type);
         }
      }
   }
   ...
   // the local copy of 'int_num'
      ...
      MPI::COMM_WORLD.Recv(int_num,1,MPI::INT,
               root, type,status);
   }
...
void reduce(double *sum, double *partial_sum,int root, int rank,int numtasks)
{
   ...
   if (rank==root) {
      *sum = *partial_sum;
      for(int mpitask=0;mpitask<numtasks;mpitask++) {
         if (mpitask!=root) {
            MPI::COMM_WORLD.Recv(partial_sum,1,MPI::DOUBLE,
            mpitask,type,status);
            (*sum) += (*partial_sum);
         } 
      }
   ...
   // if not root MPI root process then send partial sum to the root  
      ...
      MPI::COMM_WORLD.Send(partial_sum,1, MPI::DOUBLE, 
         root, type);
   }
}
...
int main( int argc, char *argv[])
{
   ...
   numtasks=MPI::COMM_WORLD.Get_size(); //get total number of processes
   ...
   // The official way of using the heap to declare a variable size array
   // Similar to the g++ extention which allows run time declarations 
