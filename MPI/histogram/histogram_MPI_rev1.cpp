...
*/
void broadcast_scalar(void *number,MPI_Datatype mpi_datatype,int root, int rank, int numprocs) {
   MPI_Status status;
   ...
   // the local copy of 'num'
      ...
      MPI_Recv(number,1,mpi_datatype,root, MPI_ANY_TAG,MPI_COMM_WORLD,&status);
...
*/
void reduce(int *histogram, int *histogram_local,int histogram_size,int root, int rank,int numprocs) {
   int tag = 123;
...
int main(int argc, char *argv[]) {
   ...
   try {
      //cout << "numbers_local_size on rank " << rank << " = " << numbers_local_array_sz << endl;
   ...
   const int local_list_size = ceil(list_size/numprocs);
