...
void compute_temp() {
    ...
    MPI_Pack_size(1,MPI_DOUBLE,MPI_COMM_WORLD,&size1);
    ...
    double *com_buf = new double [com_buf_size];
    MPI_Buffer_attach(com_buf,com_buf_size);
    ...
    for (int i=0;i<num_iterations;i++) {
        ...
        //std::cout << "Beginning interation " << i << " Up = " << up_pr << " Down = " << down_pr << std::endl;

        // Send Up and down
        if (rank < numprocs-1){
            //std::cout << "Proc " << rank << " sending down: " << down_pr << std::endl;
            MPI_Bsend(&temp[active_rows_on_proc*total_cols],total_cols,MPI_DOUBLE,down_pr,123,MPI_COMM_WORLD);
        }
        if (rank != 0){
            //std::cout << "Proc " << rank << " sending up: " << up_pr << std::endl;
            MPI_Bsend(&temp[total_cols],total_cols,MPI_DOUBLE,up_pr,123,MPI_COMM_WORLD);
        }

        // Receive Up and Down
        if (rank < numprocs-1){
            //std::cout << "Proc " << rank << " receiving down " << down_pr << std::endl;
            MPI_Recv(&temp[(active_rows_on_proc+1)*total_cols],total_cols,MPI_DOUBLE,down_pr,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
        }
        if (rank != 0){
            //std::cout << "Proc " << rank << " receiving up " << up_pr << std::endl;
