// 1-D temperature Example with the combined MPI_Sendrec Routine being used on 
// once for both incoming and outgoing communications on both the left and right
// side of the communication blocks (except at the rank 0 and rank numprocs-1 
// cases.
using namespace std;
#include <stdlib.h>
#include <iostream>
...
void compute_temp(double * temp, int total_points, int num_iterations,
   ...
   for (int i=0;i<num_iterations;i++) {
      // if not last process the Send/Rec to/from rightmost adjacent Process
      if (rank < numprocs-1) {
         MPI_Sendrecv(&temp[points_per_process],1,MPI_DOUBLE,right_pr,123,
         &temp[points_per_process+1],1,MPI_DOUBLE,right_pr,123,MPI_COMM_WORLD,&status);
      }
      // if not first (rank 0) process Send/Rec to/from leftmost adjacent process
      if (rank > 0) {
            MPI_Sendrecv(&temp[1],1,MPI_DOUBLE,left_pr,123,
            &temp[0],1,MPI_DOUBLE,left_pr,123,MPI_COMM_WORLD,&status);
      }
