...
int main (int argc, char *argv[]){
    ...
    if (n % numprocs == 0){
        ...
        if(rank != numprocs-1){
            ...
            active_rows_on_proc = ceilVal;

        } else { // The last proc gets the left over rows

            int leftovers = n;
            while (leftovers > ceilVal){
                leftovers -= ceilVal;
            }
  
            active_rows_on_proc = leftovers;
        }
    }
